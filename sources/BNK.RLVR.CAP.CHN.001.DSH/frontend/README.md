# BNK.RLVR.CAP.CHN.001.DSH — Frontend shell (TASK-002)

Vanilla HTML5 / CSS3 / JS frontend shell for the **Beneficiary Dashboard**
capability. Pairs with the .NET 10 BFF under
sibling `bff/` folder under `sources/BNK.RLVR.CAP.CHN.001.DSH/` (scaffolded in parallel by the `create-bff`
agent).
...
```
sources/BNK.RLVR.CAP.CHN.001.DSH/frontend/
...
```bash
cd sources/BNK.RLVR.CAP.CHN.001.DSH/frontend
...
`process/BNK.RLVR.CAP.CHN.001.DSH/read-models.yaml`) and the footer indicator
