...
/// BNK.RLVR.RVT.BSP.001.TIER_UPGRADE_RECORDED on bsp.001.tie-events
/// (binding BNK.RLVR.EVT.BSP.001.TIER_UPGRADED.BNK.RLVR.RVT.BSP.001.TIER_UPGRADE_RECORDED).
///
/// Tier downgrades (BNK.RLVR.RVT.BSP.001.TIER_DOWNGRADE_RECORDED) are NOT
...
public sealed class OnTierUpgradeRecordedConsumer : IConsumer<TierUpgradeRecordedRaw>
{
    ...
    public Task Consume(ConsumeContext<TierUpgradeRecordedRaw> context)
    {
        ...
        if (!_schema.ValidateTier(parsed, out var schemaError))
        {
            ...
            throw new InvalidOperationException(
                $"BNK.RLVR.RVT.BSP.001.TIER_UPGRADE_RECORDED schema validation failed: {schemaError}. " +
                "Routing to DLQ.");
        }
