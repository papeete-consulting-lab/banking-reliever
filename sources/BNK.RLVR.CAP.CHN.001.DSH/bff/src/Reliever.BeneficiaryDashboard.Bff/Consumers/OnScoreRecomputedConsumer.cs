...
/// BNK.RLVR.RVT.BSP.001.CURRENT_SCORE_RECOMPUTED on exchange bsp.001.sco-events
/// (binding key BNK.RLVR.EVT.BSP.001.SCORE_RECOMPUTED.BNK.RLVR.RVT.BSP.001.CURRENT_SCORE_RECOMPUTED).
///
/// Behaviour:
...
///      (process/BNK.RLVR.CAP.BSP.001.SCO/schemas/…) → throws on shape mismatch (→ DLQ).
///   4. Deserialises into <see cref="ScoreRecomputedPayload"/>.
///   5. Maps to <see cref="SynchronizeScoreCommand"/> per the mapping rule
///      declared in process/BNK.RLVR.CAP.CHN.001.DSH/policies.yaml:ON_SCORE_RECOMPUTED.
...
public sealed class OnScoreRecomputedConsumer : IConsumer<ScoreRecomputedRaw>
{
    ...
    public Task Consume(ConsumeContext<ScoreRecomputedRaw> context)
    {
        ...
        if (!_schema.ValidateScore(parsed, out var schemaError))
        {
            ...
            throw new InvalidOperationException(
                $"BNK.RLVR.RVT.BSP.001.CURRENT_SCORE_RECOMPUTED schema validation failed: {schemaError}. " +
                "Routing to DLQ.");
        }
